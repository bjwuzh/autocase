...
class GenerateCase:
    readdata = read_case_config.ReadData()
    ...
    @classmethod
    def get_normal_values(self, params):
        normal_values = []
        for param in params:
            values = param['values']
    ...
    @classmethod
    def get_src_file(self,case_dir):
        # 目前只支持第一个文件
    ...
    @classmethod
    def generate(self, cases_dir):
        origin_header = ['接口名称', '接口', '测试场景', '是否完成', '编写完成日期', '优先级']
        header = []
        header.extend(origin_header)
        case_list = [header]
        src_json = self.readdata.read_excel(self.get_src_file(cases_dir))
        ...
        normal_values = self.get_normal_values(body)
...
if __name__ == '__main__':
    gc = GenerateCase()
    print(gc.generate('F:\\code\\itest\\autocase\\data'))