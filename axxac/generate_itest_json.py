...
from axxac import generate_case_list

class Generatejson:
    # 加载itest中导出的目标json格式的文件
    # json_file：传入一个从itest中导出的json文件
    # @classmethod
    # def read_itest_json(self,json_file):
    ...
    # @classmethod
    # def get_header_list(self,header):
    #     header_list = []
    ...
    # @classmethod
    # def get_extract_list(self,extract):
    #     extract_list = []
    ...
    @classmethod
    def get_assert_list(self,the_assert):
        assert_list = []
    ...
    @classmethod
    def generate_header_item(self,row):
    ...
    # @classmethod
    # def generate_extract_item(self,row):
    ...
    @classmethod
    def generate_extract_item_null(self):
        extract_item = OrderedDict([
            ("type", "JSONPath"),
            ("variable", ""),
            ("value", ""),
    ...
    @classmethod
    def generate_request_item(self,header, method, title, row, the_assert, extract):
        raw = dict()
        querys_items = []
        for i in range(6, len(row)):
            value = row[i]
            if self.is_json_str(value):
        ...
        assert_list = self.get_assert_list(the_assert)
        # 每一个header
        header_items = []
        for row_num in range(0, len(header)):
            header_item = self.generate_header_item(header[row_num])
            header_items.append(header_item)

        # 每一个提取
        extract_items = []
        for row_num in range(0, len(extract)):
            extract_item = self.generate_extract_item_null()
    ...
    @classmethod
    def generate(self,result_json, output_dir):
        ...
        # 读取itest导出的json文件
        # itest_json = self.read_itest_json(get_cur_dir() + '/itest_template.json')
        itest_json = []
        ...
        for row_num in range(1, len(case_list)):
            ...
            case_item = self.generate_request_item(header, method, case_list[0], case_list[row_num], the_assert, extract)
